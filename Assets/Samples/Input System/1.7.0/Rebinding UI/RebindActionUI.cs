...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        {
            foreach (var action in m_Action.action.bindings)
            {
                if (action.path.ToLower().Contains("gamepad"))
                {
                    var currentBindings = actionReference.action.actionMap.SaveBindingOverridesAsJson();
                    PlayerPrefs.SetString($"{action.name}_gamepad", currentBindings);
                }
                else
                {
                    var currentBindings = actionReference.action.actionMap.SaveBindingOverridesAsJson();
                    PlayerPrefs.SetString($"{action.name}_keyboard", currentBindings);
                }
            }
        }
